...
impl GetCameraModel for MarsImage {
    ...
    fn implements_linearized(&self) -> bool {
        match self.get_camera_model() {
            Some(c) => {
                if let Ok(_) = c.linearize(self.image.width, self.image.height, self.image.width, self.image.height) {
                    true
                } else {
                    false
                }
...
fn project_line_sample(img:&RgbImage, input_model:&CameraModel, output_model:&Cahv, line:usize, sample:usize, map:&mut RgbImage, x_offset:usize) {
    let ground = Vector::new(0.0, 0.0, -1.84566);

    if let Ok(lv) = input_model.ls_to_look_vector(&ImageCoordinate{line: line as f64, sample: sample as f64}) {
        let ls_in = match lv.intersect_to_plane(&ground) {
            Some(ray) => {
                output_model.xyz_to_ls(&ray, false)
            },
            None => {
                output_model.xyz_to_ls(&lv.look_direction, true)
            }
        };
...
fn linearize_create(left_img:&MarsImage, right_img:&MarsImage, map:&mut RgbImage) {
    ...
    let left_input_cahv = left_img.get_camera_model().expect("No left CAHV after all...");
    let left_output_cahv = left_input_cahv.linearize(left_img.image.width, left_img.image.height, left_img.image.width, left_img.image.height).expect("Failed to linearize left CAHV");
    
    let right_input_cahv = right_img.get_camera_model().expect("No right CAHV after all...");
    let right_output_cahv = right_input_cahv.linearize(right_img.image.width, right_img.image.height, right_img.image.width, right_img.image.height).expect("Failed to linearize right CAHV");

    for y in 0..left_img.image.height {
        for x in 0..left_img.image.width {
            project_line_sample(&right_img.image, &right_input_cahv, &right_output_cahv, y, x, map, 0);
            project_line_sample(&left_img.image, &left_input_cahv, &left_output_cahv, y, x, map, left_img.image.width);
            project_line_sample(&right_img.image, &right_input_cahv, &right_output_cahv, y, x, map, left_img.image.width * 2);
        }
    }
