...
pub fn process_image(img:&MarsImage, map:&mut RgbImage, input_model:&CameraModel, output_model:&Cahv, ground:&Vector, eye:Eye) {
        ...
        for y in 0..map.height {
            ...
            if let Ok(lv) = output_model.ls_to_look_vector(&ImageCoordinate{line: y as f64, sample: x as f64}) {
                ...
                let ls_in = match ray {
                    ...
                    None => {
                        input_model.xyz_to_ls(&lv.look_direction, true)
                    }
                };
                ...
                if ls_in.sample >= 0.0 && ls_in.line >= 0.0 && in_x < img.image.width - 1 && in_y < img.image.height - 1 {
                    let tl = Point::create(
                        x as f64,
                        y as f64,
